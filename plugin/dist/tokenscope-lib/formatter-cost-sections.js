...
export function formatCostEstimateLines(cost) {
    ...
    for (const modelCost of cost.perModelCosts) {
        lines.push(`  ${modelCost.modelName} (${modelCost.apiCallCount} call${modelCost.apiCallCount === 1 ? "" : "s"}): $${modelCost.estimatedSessionCost.toFixed(4)}`);
    ...
    if (cost.cacheReadTokens > 0)
        lines.push(`  Blended cache read: $${cost.estimatedCacheReadCost.toFixed(4)}`);
    if (cost.cacheWriteTokens > 0)
        lines.push(`  Blended cache write: $${cost.estimatedCacheWriteCost.toFixed(4)}`);
    return lines;
}
export function formatDetailedSubagentBreakdownLines(breakdown, indent = "    ") {
...
function formatSingleModelCostLines(cost) {
    const modelCost = cost.perModelCosts[0];
    if (modelCost)
        return formatModelCostTokenLines(modelCost, "  ");
    return [
...
function formatModelCostTokenLines(modelCost, indent) {
    const lines = [];
    ...
    lines.push(`${indent}Input tokens:      ${formatNumber(modelCost.inputTokens).padStart(10)} × $${modelCost.pricePerMillionInput.toFixed(2)}/M  = $${modelCost.estimatedInputCost.toFixed(4)}`);
    lines.push(`${indent}Output tokens:     ${formatNumber(modelCost.outputTokens + modelCost.reasoningTokens).padStart(10)} × $${modelCost.pricePerMillionOutput.toFixed(2)}/M  = $${modelCost.estimatedOutputCost.toFixed(4)}`);
    if (modelCost.cacheReadTokens > 0) {
        lines.push(`${indent}Cache read:        ${formatNumber(modelCost.cacheReadTokens).padStart(10)} × $${modelCost.pricePerMillionCacheRead.toFixed(2)}/M  = $${modelCost.estimatedCacheReadCost.toFixed(4)}`);
    }
    if (modelCost.cacheWriteTokens > 0) {
        lines.push(`${indent}Cache write:       ${formatNumber(modelCost.cacheWriteTokens).padStart(10)} × $${modelCost.pricePerMillionCacheWrite.toFixed(2)}/M  = $${modelCost.estimatedCacheWriteCost.toFixed(4)}`);
    }
    return lines;
}
//# sourceMappingURL=formatter-cost-sections.js.map