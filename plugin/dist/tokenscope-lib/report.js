...
export async function writeReport(outputPath, output) {
    try {
        try {
...
export function buildFailureReport(sessionID, warnings, fatalMessage) {
    const lines = [];
...
export function buildSuccessSummary(outputPath, analysis) {
    const timestamp = new Date().toISOString();
    const localEstimatedTokens = analysis.totalTokens;
    const mainSessionTelemetryTokens = analysis.inputTokens +
    ...
    if (analysis.subagentAnalysis && analysis.subagentAnalysis.subagents.length > 0) {
        ...
        const grandTotal = new Intl.NumberFormat("en-US").format(mainSessionTelemetryTokens + analysis.subagentAnalysis.totalTokens);
