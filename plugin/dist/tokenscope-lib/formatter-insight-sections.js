import { calculateModelAwareCacheEfficiency, formatContextBar, formatEfficiencyBar, formatNumber, SKILL_DESC_WIDTH, SKILL_NAME_WIDTH, SUBAGENT_DESC_WIDTH, SUBAGENT_NAME_WIDTH, TOOL_ESTIMATE_LABEL_WIDTH, } from "./formatter-helpers.js";
export function formatContextBreakdown(breakdown) {
    const lines = [];
...
export function formatToolEstimates(estimates) {
    const lines = [];
    ...
    lines.push(``);
    lines.push(`═══════════════════════════════════════════════════════════════════════════`);
    lines.push(usesOpenCodeMetadata
    ...
    lines.push(`  ───────────────────────────────────────────────────────────────────`);
    lines.push(`  Total:${" ".repeat(TOOL_ESTIMATE_LABEL_WIDTH - 6)} ~${formatNumber(totalTokens).padStart(11)} tokens (${enabledEstimates.length} enabled tools)`);
    lines.push(``);
...
export function formatCacheEfficiency(efficiency, cost, modelName) {
    const lines = [];
    ...
    if (efficiency.cacheWriteTokens > 0) {
        ...
        lines.push(`    Cache Write:      ${formatNumber(efficiency.cacheWriteTokens).padStart(10)} tokens   ${cacheWriteBar}  ${cacheWritePct}%`);
    }
    lines.push(`  ───────────────────────────────────────────────────────────────────`);
    ...
    if (cost.perModelCosts.length > 1) {
        ...
        const tierLabel = modelCost?.pricingTier === "context_over_200k"
            ? ", 200K+ context rates"
            : modelCost?.pricingTier === "mixed_context_tiers"
                ? ", mixed context rates"
                : "";
        lines.push(`  Cost Analysis (${modelName} @ $${inputRate.toFixed(2)}/M input, $${cacheReadRate.toFixed(2)}/M cache read, $${cacheWriteRate.toFixed(2)}/M cache write${tierLabel}):`);
        lines.push(`    Without caching:   $${modelAwareEfficiency.costWithoutCaching.toFixed(4)}  (${formatNumber(total)} tokens x $${uncachedInputRate.toFixed(2)}/M)`);
        lines.push(`    With caching:      $${modelAwareEfficiency.costWithCaching.toFixed(4)}  (fresh x $${inputRate.toFixed(2)}/M + cache read x $${cacheReadRate.toFixed(2)}/M + cache write x $${cacheWriteRate.toFixed(2)}/M)`);
    }
    lines.push(`  ───────────────────────────────────────────────────────────────────`);
    lines.push(`  Cost Savings:        $${modelAwareEfficiency.costSavings.toFixed(4)}  (${modelAwareEfficiency.savingsPercent.toFixed(1)}% reduction)`);
    lines.push(`  Effective Rate:      $${modelAwareEfficiency.effectiveRate.toFixed(2)}/M tokens  (vs. $${modelAwareEfficiency.standardRate.toFixed(2)}/M standard)`);
    return lines;
}
export function formatAvailableSkills(analysis) {
    const lines = [];
    ...
    for (const skill of sortedSkills) {
        const name = skill.name.length > SKILL_NAME_WIDTH
            ? skill.name.substring(0, SKILL_NAME_WIDTH - 1) + "…"
            : skill.name.padEnd(SKILL_NAME_WIDTH);
        const desc = skill.description.length > SKILL_DESC_WIDTH
    ...
    lines.push(``);
    if (hasSystemPromptCatalog) {
        lines.push(`  Note: Full system-prompt skill catalog is ~${formatNumber(analysis.availableSkillsContextTokens)} tokens (includes shared wrapper/preamble).`);
        if (analysis.skillToolDescriptionTokens > 0) {
            lines.push(`        Compact skill tool description adds ~${formatNumber(analysis.skillToolDescriptionTokens)} tokens more.`);
        }
    }
    else if (analysis.skillToolDescriptionTokens > 0) {
        lines.push(`  Note: Full skill tool description is ~${formatNumber(analysis.skillToolDescriptionTokens)} tokens (includes boilerplate).`);
    }
    return lines;
}
export function formatLoadedSkills(analysis) {
    const lines = [];
    ...
    lines.push(`  ─────────────────────────────────────────────────────`);
    lines.push(`  Total: ${formatNumber(total)} tokens (${analysis.loadedSkills.length} skill${analysis.loadedSkills.length !== 1 ? "s" : ""} loaded)`);
    lines.push(``);
    lines.push(`  Note: Loaded skill content stays in context (protected from pruning).`);
    return lines;
}
export function formatAvailableSubagents(analysis) {
    const lines = [];
    ...
    for (const subagent of sortedSubagents) {
        const name = subagent.name.length > SUBAGENT_NAME_WIDTH
            ? subagent.name.substring(0, SUBAGENT_NAME_WIDTH - 1) + "…"
            : subagent.name.padEnd(SUBAGENT_NAME_WIDTH);
        const desc = subagent.description.length > SUBAGENT_DESC_WIDTH
    ...
    lines.push(`  ───────────────────────────────────────────────────────────────────────`);
    lines.push(`  Total: ~${formatNumber(total)} tokens (${analysis.availableSubagents.length} subagents available)`);
    lines.push(``);
    lines.push(`  Note: Full task tool description is ~${formatNumber(analysis.taskToolDescriptionTokens)} tokens (includes instructions/examples).`);
    return lines;
}
//# sourceMappingURL=formatter-insight-sections.js.map