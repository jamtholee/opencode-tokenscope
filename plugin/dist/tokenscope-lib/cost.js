...
export class CostCalculator {
    ...
    calculateCost(analysis) {
        ...
        const hasActivity = analysis.apiCallCount > 0 &&
    ...
    calculatePerModelCosts(analysis, fallbackPricingModelName) {
    ...
    calculateModelUsageCost(modelUsage, pricing) {
        ...
        const pricingTier = segmentCosts.every((segment) => segment.pricingTier === firstTier)
            ? firstTier
            : "mixed_context_tiers";
        const summedCosts = this.sumCostBreakdowns(segmentCosts);
        const lastSegmentCost = segmentCosts[segmentCosts.length - 1];
        const mixedRate = (tokens, cost, fallback) => pricingTier === "mixed_context_tiers" && tokens > 0 ? (cost / tokens) * 1_000_000 : fallback;
        return {
            ...summedCosts,
            ...
            pricePerMillionOutput: mixedRate(modelUsage.outputTokens + modelUsage.reasoningTokens, summedCosts.estimatedOutputCost, lastSegmentCost?.pricePerMillionOutput ?? pricing.output),
            pricePerMillionCacheRead: mixedRate(modelUsage.cacheReadTokens, summedCosts.estimatedCacheReadCost, lastSegmentCost?.pricePerMillionCacheRead ?? pricing.cacheRead),
            pricePerMillionCacheWrite: mixedRate(modelUsage.cacheWriteTokens, summedCosts.estimatedCacheWriteCost, lastSegmentCost?.pricePerMillionCacheWrite ?? pricing.cacheWrite),
            pricingTier,
        };
    }
    sumCostBreakdowns(costs) {
    ...
    calculateUsageCostForRate(modelUsage, rate, basePricing) {
    ...
    calculateUncachedInputCost(modelUsage, pricing) {
    ...
    selectPricingRate(modelUsage, pricing) {
        const contextTokens = modelUsage.inputTokens + modelUsage.cacheReadTokens + modelUsage.cacheWriteTokens;
        if (pricing.contextOver200k && contextTokens > (pricing.contextOver200k.threshold ?? 200_000))
            return pricing.contextOver200k;
        return pricing;
    }
    resolvePricingModelName(modelUsage, fallbackPricingModelName) {
