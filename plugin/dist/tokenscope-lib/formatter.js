...
import { collectTopEntries, formatCategoryBar, formatNumber, TOOL_LABEL_WIDTH, TOP_CONTRIBUTOR_LABEL_WIDTH, } from "./formatter-helpers.js";
import { formatAvailableSkills, formatAvailableSubagents, formatCacheEfficiency, formatContextBreakdown, formatLoadedSkills, formatToolEstimates, } from "./formatter-insight-sections.js";
export class OutputFormatter {
    ...
    format(analysis) {
        ...
        return this.formatVisualOutput(analysis.sessionID, analysis.model.name, analysis.warnings, analysis.totalTokens, analysis.inputTokens, analysis.outputTokens, analysis.reasoningTokens, analysis.cacheReadTokens, analysis.cacheWriteTokens, analysis.assistantMessageCount, analysis.apiCallCount, analysis.callsWithCacheRead, analysis.callsWithCacheWrite, analysis.mostRecentInput, analysis.mostRecentOutput, analysis.mostRecentReasoning, analysis.mostRecentCacheRead, analysis.mostRecentCacheWrite, analysis.mostRecentProviderTotalTokens, inputCategories, outputCategories, topEntries, toolEntries, hasInferredSystemEstimate, costEstimate, analysis.subagentAnalysis, analysis.contextBreakdown, analysis.toolEstimates, analysis.cacheEfficiency, analysis.skillAnalysis);
    }
    formatVisualOutput(sessionID, modelName, warnings, totalTokens, inputTokens, outputTokens, reasoningTokens, cacheReadTokens, cacheWriteTokens, assistantMessageCount, apiCallCount, callsWithCacheRead, callsWithCacheWrite, mostRecentInput, mostRecentOutput, mostRecentReasoning, mostRecentCacheRead, mostRecentCacheWrite, mostRecentProviderTotalTokens, inputCategories, outputCategories, topEntries, toolEntries, hasInferredSystemEstimate, cost, subagentAnalysis, contextBreakdown, toolEstimates, cacheEfficiency, skillAnalysis) {
        const lines = [];
        ...
        lines.push(`  \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500`);
        lines.push(`  Total:             ${formatNumber(mostRecentInput + mostRecentCacheRead + mostRecentCacheWrite + mostRecentOutput + mostRecentReasoning).padStart(10)} tokens`);
        // 5. SESSION TOTALS
        lines.push(``);
        lines.push(`\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550`);
        ...
        if (subagentAnalysis && subagentAnalysis.subagents.length > 0) {
            ...
            for (const subagent of subagentAnalysis.subagents) {
                ...
                if (this.config?.enableDetailedSubagentCostBreakdown) {
                    lines.push(...formatDetailedSubagentBreakdownLines({
            ...
            lines.push(`\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500`);
            lines.push(`Subagent Total:${" ".repeat(subagentLabelWidth - 14)} $${subagentTotalCost.toFixed(4)}  (${formatNumber(subagentAnalysis.totalTokens)} tokens, ${subagentAnalysis.totalApiCalls} calls)`);
            if (this.config?.enableDetailedSubagentCostBreakdown) {
                lines.push(...formatDetailedSubagentBreakdownLines({
        ...
        if (subagentAnalysis && subagentAnalysis.subagents.length > 0) {
            ...
            lines.push(`  Main session:      $${mainCost.toFixed(4).padStart(10)}    ${formatNumber(sessionTotal).padStart(10)}         ${apiCallCount.toString().padStart(5)}`);
            lines.push(`  Subagents:         $${subagentTotalCost.toFixed(4).padStart(10)}    ${formatNumber(subagentAnalysis.totalTokens).padStart(10)}         ${subagentAnalysis.totalApiCalls.toString().padStart(5)}`);
            lines.push(`\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500`);
            lines.push(`  TOTAL:             $${grandTotalCost.toFixed(4).padStart(10)}    ${formatNumber(grandTotalTokens).padStart(10)}         ${grandTotalApiCalls.toString().padStart(5)}`);
        }
        else {
            lines.push(`  Session:           $${mainCost.toFixed(4).padStart(10)}    ${formatNumber(sessionTotal).padStart(10)}         ${apiCallCount.toString().padStart(5)}`);
        }
        lines.push(``);
        lines.push(`\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550`);
        return lines.join("\n");
