...
export class SubagentAnalyzer {
    ...
    warnings;
    routing;
    constructor(client, costCalculator, warnings, routing = {}) {
        this.client = client;
    ...
    async analyzeChildSessions(parentSessionID) {
        ...
        try {
            ...
            const children = unwrapResponseData(childrenResponse ?? []);
            ...
            this.warnings?.add(`Subagent analysis was skipped for session ${parentSessionID}: ${formatErrorMessage(error)}`, `subagent-root:${parentSessionID}`);
        }
        return result;
    }
    async analyzeChildSession(child) {
        try {
            ...
            const messages = unwrapResponseData(messagesResponse ?? []);
            ...
            for (const message of messages) {
                if (message.info.role !== "assistant")
                    continue;
                const messageProviderID = message.data?.providerID ??
                    message.data?.model?.providerID ??
                    message.info.providerID ??
                    message.info.model?.providerID ??
                    message.providerID ??
                    message.model?.providerID;
                const messageModelID = message.data?.modelID ??
            ...
            const estimatedCosts = telemetry.perModelUsage.reduce((sum, modelUsage) => {
                ...
                if (!this.costCalculator.hasPricing(pricingModelName)) {
                    this.warnings?.add(`Pricing for child session model '${pricingModelName}' was not found in OpenCode metadata or models.json. Subagent cost estimates for that model use the default fallback rates ($1/M input, $3/M output, no cache pricing).`, `missing-subagent-pricing:${pricingModelName}`);
                }
            ...
            this.warnings?.add(`A child session could not be analyzed (${child.id}): ${formatErrorMessage(error)}`, `subagent-child:${child.id}`);
            return null;
        }
    }
