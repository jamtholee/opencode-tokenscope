...
export function formatCategoryBar(label, tokens, total, labelWidth = CATEGORY_LABEL_WIDTH) {
    if (tokens === 0)
        return "";
...
export function formatContextBar(label, tokens, total) {
    const percentage = total > 0 ? ((tokens / total) * 100).toFixed(1) : "0.0";
    const percentageNum = parseFloat(percentage);
    const barWidth = Math.round((percentageNum / 100) * BAR_WIDTH);
    const bar = "█".repeat(barWidth) + "░".repeat(Math.max(0, BAR_WIDTH - barWidth));
...
export function calculateModelAwareCacheEfficiency(efficiency, cost) {
    if (cost.perModelCosts.length === 0)
        return efficiency;
    ...
    const costWithCaching = cost.perModelCosts.reduce((sum, modelCost) => sum + modelCost.estimatedInputCost + modelCost.estimatedCacheReadCost + modelCost.estimatedCacheWriteCost, 0);
    ...
    const standardRate = efficiency.totalInputTokens > 0 ? (costWithoutCaching / efficiency.totalInputTokens) * 1_000_000 : efficiency.standardRate;
    return {
        ...efficiency,
        costWithoutCaching,
        costWithCaching,
...
//# sourceMappingURL=formatter-helpers.js.map