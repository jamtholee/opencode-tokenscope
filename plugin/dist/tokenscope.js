...
import { addModelSupportWarnings, addPerModelPricingWarnings, attachConfiguredAnalyses, resolveSessionID, } from "./tokenscope-lib/session-workflow.js";
export const TokenAnalyzerPlugin = async ({ client, serverUrl, directory }) => {
    ...
    return {
        tool: {
            tokenscope: tool({
                description: "Analyze token usage across the current session with detailed breakdowns by category (system, user, assistant, tools, reasoning). " +
                ...
                async execute(args, context) {
                    ...
                    try {
                        ...
                        const messages = unwrapResponseData(response ?? []);
                        ...
                        const analysis = await analysisEngine.analyze(sessionID, messages, tokenModel, args.limitMessages ?? DEFAULT_ENTRY_LIMIT);
                        ...
                        warnings.add(`TokenScope returned a fallback report instead of aborting the session: ${fatalMessage}`, `fatal:${sessionID}`);
